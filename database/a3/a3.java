...
public class a3 {
	...
	public static void main (String[] args) throws Exception{ 
		...
		// 				}
		// 			}
		// 			industryReturn = industryReturn * (1.0/(basket.size()-1.0))-1.0;
		// 			writer.printf("%s  %s  %s  %s  %10.7f %10.7f\n",
		// 				anIndustry, outKey, outComp.getDate(i), outComp.getDate(i+59), tickerReturn, industryReturn);
		// 			industryReturn = 0.0;
		// 		}

		// 	}	

		// 	basket = new LinkedHashMap<String, PriceVolume>();
		// }
		// writer.close();

		getDates.setString(1, "Information Technology");
		dateRange = getDates.executeQuery();
		dateRange.next();
		sDate = dateRange.getString(1);
		eDate = dateRange.getString(2);

		getTickers.setString(1, "Information Technology");
		tickers = getTickers.executeQuery();
		while (tickers.next()) {
			aTicker = tickers.getString("Ticker");
			adjpv = getAdjPV(aTicker);
		...
		for (String outKey : basket.keySet()) {
			...
			for (int i = s; i < e - (e-s)%60; i = i + 60){
				...
				industryReturn = industryReturn * (1.0/(basket.size()-1.0))-1;
				writer.printf("%s  %s  %s  %10.7f %10.7f\n",
					outKey, outComp.getDate(i), outComp.getDate(i+59), tickerReturn, industryReturn);
				industryReturn = 0.0;
			}
